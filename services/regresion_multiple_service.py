...
def obtener_datos_ventas_multiple():
    ...
    try:
        ...
        columnas_necesarias = {'tipo', 'cantidad', 'total'}
        ...
        if 'fecha' in df.columns:
            ...
            # Para fechas inválidas, rellenar con la fecha actual
            df['fecha'].fillna(pd.Timestamp.now(), inplace=True)
        else:
            # Si no hay fecha, usar la fecha actual para todos
        ...
        X = df[['cantidad', 'tipo_codigo', 'dia_semana']].values.astype(np.float32)
