...
tipos = [
    row["tipo"]
    for row in df.select("tipo").distinct().collect()
]
...
resumen_pd = resumen_pd.dropna(subset=["tipo"])
resumen_pd["tipo"] = resumen_pd["tipo"].astype(str)

st.dataframe(resumen_pd)



st.subheader("Ingreso Total por Tipo de Café")
