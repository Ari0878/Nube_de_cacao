...
from services.kmeans_service import entrenar_kmeans, calcular_clusters_optimos, predecir_cluster
from services.decision_tree_service import entrenar_arbol_regresion, entrenar_arbol_clasificacion, predecir_con_arbol
...
def historial():
    ventas = list(ventas_col.find().sort("fecha", -1))
    for v in ventas:
        v["_id"] = str(v["_id"])
...
def kmeans():
    ...
    clusters_optimos = None
    prediccion = None
    
    if request.method == "POST":
        if "calcular_optimos" in request.form:
            clusters_optimos = calcular_clusters_optimos()
        elif modelo:
            try:
                cantidad = float(request.form.get("cantidad", 0))
                total = float(request.form.get("total", 0))
                prediccion = predecir_cluster(cantidad, total)
            except:
                pass
    
    return render_template("kmeans.html", modelo=modelo, clusters_optimos=clusters_optimos, prediccion=prediccion)


# ======================== ÁRBOL DE DECISIÓN ========================
@app.route("/arbol-decision", methods=["GET", "POST"])
def arbol_decision():
    if not session.get("logged_in"):
        return redirect("/login")
    
    arbol_regresion = entrenar_arbol_regresion()
    arbol_clasificacion = entrenar_arbol_clasificacion()
    prediccion = None
    
    if request.method == "POST" and arbol_regresion:
        try:
            cantidad = float(request.form.get("cantidad", 0))
            ...
            prediccion = predecir_con_arbol(cantidad, precio_unitario, dia_semana, mes)
        except:
            pass
    
    return render_template("arbol_decision.html", 
                         arbol_regresion=arbol_regresion,
                         arbol_clasificacion=arbol_clasificacion,
